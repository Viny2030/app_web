...
try:
    ...
    try:
        resultados = json.loads('{"tipo_problema": "clustering_o_anomalias", "clusters_encontrados": 3, "metodo": "KMeans"}')
        st.json(resultados)
    except Exception as e:
        st.error(f"Error al cargar los resultados: {str(e)}")
        st.code('{"tipo_problema": "clustering_o_anomalias", "clusters_encontrados": 3, "metodo": "KMeans"}')

    # ----------------------------------------------------------------------
    # 💡 SECCIÓN DE GRÁFICOS
    # ----------------------------------------------------------------------
    
    tipo_problema = "clustering_o_anomalias"
